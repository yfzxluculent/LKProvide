...
+ (UIImage *)qrImageFromString:(NSString *)qrString sideLength:(CGFloat)length
                    CenterIcon:(UIImage *)icon{
    
    
    UIImage *qrImage = [UIImage qrImageFromString:qrString sideLength:length];
    
    UIImage *centerIcon = icon;
    
    if (!icon) {
        centerIcon = HT_IMAGENAME(@"qr_icon");
    }
    ...
    UIGraphicsBeginImageContextWithOptions(qrImage.size, YES, [UIScreen mainScreen].scale);
    [qrImage drawInRect:CGRectMake(0, 0, qrImage.size.width, qrImage.size.height)];
    
    CGFloat imageW = length / 3;
    CGFloat imageX = (qrImage.size.width - imageW) * 0.5;
    CGFloat imgaeY = (qrImage.size.height - imageW) * 0.5;
    
    [centerIcon drawInRect:CGRectMake(imageX, imgaeY, imageW, imageW)];
...
+ (NSString *)stringFromCiImage:(CIImage *)ciimage {
    ...
    if (features.count) {
        ...
        HTLog(@"未正常解析二维码图片, 请确保iphone5/5c以上的设备");
    }
