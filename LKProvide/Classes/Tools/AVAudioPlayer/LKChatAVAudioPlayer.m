...
NSString *const kLKChatAudioDataKey;

@interface LKChatAVAudioPlayer () <AVAudioPlayerDelegate,AVAudioSessionDelegate>
{
...
@implementation LKChatAVAudioPlayer
...
- (NSOperationQueue *)audioDataOperationQueue {
    if (_audioDataOperationQueue == nil) {
        ...
        audioDataOperationQueue.name = @"com.LayIM.LKChatAVAudipPlayer.loadAudioDataQueue";
...
- (void)playAudioWithURLString:(NSString *)URLString identifier:(NSString *)identifier {
    ...
    if ([self.URLString isEqualToString:URLString] && self.identifier == identifier) {
        ...
        [self setAudioPlayerState:LKChatVoiceMessageStateCancel];
        return;
    }
    ...
    NSBlockOperation *blockOperation = [NSBlockOperation blockOperationWithBlock:^{
        [self setAudioPlayerState:LKChatVoiceMessageStateDownloading];
    ...
    [blockOperation setName:[[NSString stringWithFormat:@"%@_%@",self.URLString, self.identifier] md5OfString]];
...
- (NSData *)audioDataFromURLString:(NSString *)URLString identifier:(NSString *)identifier {
    ...
    if ([URLString hasPrefix:@"http"] || [URLString hasPrefix:@"https"]) {
        ...
        NSString *audioCacheKey = [URLString md5OfString];
    ...
    if (audioData) {
        objc_setAssociatedObject(audioData, &kLKChatAudioDataKey, [[NSString stringWithFormat:@"%@_%@",URLString,identifier] md5OfString], OBJC_ASSOCIATION_COPY);
    }
...
- (void)playAudioWithData:(NSData *)audioData {
    ...
    NSString *audioURLMD5String = objc_getAssociatedObject(audioData, &kLKChatAudioDataKey);
    
    if (![[[NSString stringWithFormat:@"%@_%@",self.URLString,self.identifier] md5OfString] isEqualToString:audioURLMD5String]) {
        return;
    }
    ...
    [self setAudioPlayerState:LKChatVoiceMessageStatePlaying];
    [_audioPlayer play];
}
...
- (void)cancelOperation {
    for (NSOperation *operation in self.audioDataOperationQueue.operations) {
        if ([operation.name isEqualToString:[[NSString stringWithFormat:@"%@_%@",self.URLString, self.identifier] md5OfString]]) {
            [operation cancel];
            break;
        }
    }
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag {
    [self setAudioPlayerState:LKChatVoiceMessageStateNormal];
    //    //删除近距离事件监听
    //    [[UIDevice currentDevice] setProximityMonitoringEnabled:NO];
...
- (void)setAudioPlayerState:(LKChatVoiceMessageState)audioPlayerState {
    _audioPlayerState = audioPlayerState;
    if (_audioPlayerState == LKChatVoiceMessageStateCancel || _audioPlayerState == LKChatVoiceMessageStateNormal) {
        _URLString = nil;
        _identifier = nil;
    }
}
