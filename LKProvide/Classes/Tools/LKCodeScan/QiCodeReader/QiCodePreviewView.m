...
//        [self.layer addSublayer:maskLayer];
        ...
        _lineLayer.shadowOffset = CGSizeMake(.10, .10);
        ...
        _torchSwithButton.titleLabel.font = [UIFont boldSystemFontOfSize:14.0];
        ...
        _torchSwithButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
        [_torchSwithButton setTitleEdgeInsets:UIEdgeInsetsMake(_torchSwithButton.imageView.frame.size.height + 10 ,-_torchSwithButton.imageView.frame.size.width, 0.0,0.0)];
            [_torchSwithButton setImageEdgeInsets:UIEdgeInsetsMake(-_torchSwithButton.imageView.frame.size.height, 0.0,0.0, -_torchSwithButton.titleLabel.bounds.size.width)];
        _torchSwithButton.hidden = YES;
        [self addSubview:_torchSwithButton];
        
        // 提示语label
        _tipsLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        _tipsLabel.textColor = [[UIColor whiteColor] colorWithAlphaComponent:1];
        _tipsLabel.textAlignment = NSTextAlignmentCenter;
        _tipsLabel.font = [UIFont boldSystemFontOfSize:14.0];
        _tipsLabel.text = @"扫二维码/条码";
        ...
        CGFloat WIDTH = 48;
        _ablumnButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _ablumnButton.frame = CGRectMake(.0, .0, WIDTH, WIDTH);
        [_ablumnButton setImage:[UIImage imageNamed:@"qi_album"] forState:(UIControlStateNormal)];
        _ablumnButton.frame = CGRectMake(CGRectGetWidth(self.layer.bounds) - WIDTH - 20.0, CGRectGetMaxY(_tipsLabel.frame) + 20.0, WIDTH, WIDTH);
        _ablumnButton.layer.cornerRadius = WIDTH/2;
        _ablumnButton.backgroundColor = [UIColor colorWithWhite:.0 alpha:.4];
        [_ablumnButton addTarget:self action:@selector(albumClicked:) forControlEvents:(UIControlEventTouchUpInside)];
        _ablumnButton.hidden = YES;
        [self addSubview:_ablumnButton];
...
- (void)setShowAlbum:(BOOL)showAlbum{
    ...
    _ablumnButton.hidden = !showAlbum;
}
...
- (void)albumClicked:(UIButton *)button {
    
    if (self.didClickedAlbumBlock && self.isShowAblum) {
        self.didClickedAlbumBlock();
    }
}
